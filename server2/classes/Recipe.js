...
function Recipe(constructorObj) {
   if(constructorObj.amount && constructorObj.description && constructorObj.image && constructorObj.ingredients && constructorObj.level && constructorObj.name && constructorObj.servings && constructorObj.timeOfPreparation && constructorObj.totalTime){
   //promenio sam yield u servings

  this._amount = constructorObj.amount;
  this._description = constructorObj.description;
  this._image = constructorObj.image;
  this._ingredients = constructorObj.ingredients;
  this._level = constructorObj.level;
  this._name = constructorObj.name;
  this._servings = constructorObj.servings;
    ...
    Object.defineProperty(Recipe.prototype, 'totalTime', {
      ...
      set: (value) => {
        if (typeof totalTime == 'number') {
          if (totalTime > 0) {
          // Once the validation is passed private property _totalTime is created
          } else {
